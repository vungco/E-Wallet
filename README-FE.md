...
Stack chốt: **Nuxt 3** (Vue 3), **REST** qua **`ewallet-gateway`**, **WebSocket** qua **`ws-gateway`** (kết quả chuyển tiền async).  
...
- **Chuyển tiền:** `POST /transfers` nhận **202** + `requestId` — **không** chờ xử lý xong trên HTTP; lắng nghe **WebSocket** (`ws-gateway`) để nhận `SUCCESS` / `FAILED`, hoặc **poll** `GET` trạng thái nếu WS rớt.
- Hai base URL: **REST** (`ewallet-gateway`) và **WS** (`ws-gateway`).
...
| `NUXT_PUBLIC_WS_BASE` | URL **`ws-gateway`** (WebSocket), ví dụ `ws://localhost:8090` hoặc `wss://...` prod |

Mọi request `$fetch` / `useFetch` dùng prefix `NUXT_PUBLIC_API_BASE`. Kết nối WS (composable `useTransferSocket` hoặc tương đương) dùng `NUXT_PUBLIC_WS_BASE`.
...
| **F3** | Trang chuyển tiền: `POST` → **202** + `requestId`; mở **WS** tới `ws-gateway` nhận kết quả | `transfer-service` + `ws-gateway` |
| **F4** | Trang lịch sử: `GET` danh sách | `transfer-service` qua gateway |
| **F5** | (Tuỳ chọn) Poll `GET` theo `requestId` khi WS disconnect | API transfer |
| **F6** | (Tuỳ chọn) Auth header trên REST + token trên WS | Gateway + ws-gateway |
...
- [ ] `POST` chuyển tiền nhận **202**; sau đó nhận kết quả qua **WebSocket** (hoặc poll).
- [ ] Lịch sử có giao dịch mới khi thành công.
